import{c as v,u as F,j as i}from"./index-YMxFGQPa.js";import{B as T}from"./button-BgQ7Bx6_.js";import{T as Y}from"./textarea-CUE6CsXM.js";import{w as B,D as L,p as j,l as k,s as U,i as q,f as X,t as H,u as N,a as V,c as W,g as K,d as G}from"./demoStore-33Xb5Yd9.js";import{R as z}from"./rotate-ccw-BHkJqRPc.js";import{S as Q}from"./save-amxezsuX.js";import{c as J}from"./createLucideIcon-DJY2vV2_.js";import{C as Z}from"./check-BidkUCQ5.js";import{C as ee}from"./copy-DYPpQaEb.js";import{C as te}from"./chevron-down-DiG1vQbj.js";const ae=[["path",{d:"M14.536 21.686a.5.5 0 0 0 .937-.024l6.5-19a.496.496 0 0 0-.635-.635l-19 6.5a.5.5 0 0 0-.024.937l7.93 3.18a2 2 0 0 1 1.112 1.11z",key:"1ffxy3"}],["path",{d:"m21.854 2.147-10.94 10.939",key:"12cjpa"}]],oe=J("send",ae),ne="pwdemo:clinic:doc",ie=1,I="pwdemo:clinic:doc-change";function R(o){return`${ne}:${o}:v${ie}`}function se(o,r){const[h,d]=v.useState(()=>{if(typeof window>"u")return r;try{return localStorage.getItem(R(o))??r}catch{return r}});v.useEffect(()=>{if(typeof window>"u")return;const l=c=>{if(c.detail?.key===o)try{const f=localStorage.getItem(R(o));f!==null&&d(f)}catch{}};return window.addEventListener(I,l),()=>window.removeEventListener(I,l)},[o]);const s=v.useCallback(l=>{d(l);try{localStorage.setItem(R(o),l)}catch{}window.dispatchEvent(new CustomEvent(I,{detail:{key:o}}))},[o]),e=v.useCallback(()=>{d(r);try{localStorage.removeItem(R(o))}catch{}window.dispatchEvent(new CustomEvent(I,{detail:{key:o}}))},[o,r]);return{value:h,set:s,reset:e}}const xe=`# Primewave Mate Clinics — Riyadh
...
`;function re({clinics:o,providers:r,appointments:h,overrides:d,lang:s}){const e=[],l=new Date,c=O(l),y=B(l);e.push("# Live clinic state (auto-generated — do NOT edit, refreshes per call)"),e.push(`Generated at: ${l.toISOString()}`),e.push(""),e.push(`## Clinics (${o.length} total)`);for(const t of o){const n=t.head_id?r.find(w=>w.id===t.head_id):null,a=(t.working_hours??L)[j(c)],m=a.open?`${a.open_time}–${a.close_time}${a.break_enabled?` (break ${a.break_start}–${a.break_end})`:""}`:"CLOSED today";e.push(`- ${k(t.name,t.name_ar,s)} · ${k(t.location,t.location_ar,s)} · ${k(t.specialty,t.specialty_ar,s)}`+(n?` · head: ${k(n.name,n.name_ar,s)}`:"")+` · today: ${m}`+(t.active?"":" · INACTIVE"))}e.push("");const f=r.filter(t=>t.active),p={};for(const t of f)p[t.role]=p[t.role]?[...p[t.role],t]:[t];e.push(`## Active staff (${f.length} total)`);for(const t of["doctor","nurse","tech","admin"]){const n=p[t]??[];if(n.length!==0){e.push(`- **${t}** (${n.length}):`);for(const a of n)e.push(`  - ${k(a.name,a.name_ar,s)} · ${k(a.specialty,a.specialty_ar,s)} · ${a.phone}`)}}e.push("");const g=he(c,o,h,d);e.push(`## Today's totals (${c})`),e.push(`Across all clinics — slots ${g.totalSlots} · booked ${g.booked} · blocked ${g.blocked} · free ${g.free}`),e.push("");const b=7,x=6,S=[];{const t=new Date;t.setHours(0,0,0,0);for(let n=0;n<b;n++){const a=new Date(t);a.setDate(t.getDate()+n),S.push(O(a))}}e.push(`## Free slots — today and next ${b-1} days (per clinic)`);for(const t of o){e.push(`### ${k(t.name,t.name_ar,s)} — ${k(t.specialty,t.specialty_ar,s)}`);for(const n of S){const a=D(t,n,h,d),m=de(n,s);if(a.totalSlots===0){e.push(`- ${m}: closed`);continue}if(a.free===0){e.push(`- ${m}: FULL (${a.booked} booked${a.blocked>0?`, ${a.blocked} blocked`:""})`);continue}const w=a.freeSlots.slice(0,x).join(", "),_=a.freeSlots.length>x?` +${a.freeSlots.length-x} more`:"";e.push(`- ${m}: ${w}${_}  (${a.free} of ${a.totalSlots} free)`)}e.push("")}e.push("## This week totals (per clinic)");for(const t of o){let n=0,a=0,m=0;for(const w of y){const _=D(t,w,h,d);m+=_.totalSlots,n+=_.booked,a+=_.blocked}e.push(`- ${k(t.name,t.name_ar,s)}: slots ${m} · booked ${n} · blocked ${a} · free ${Math.max(0,m-n-a)}`)}e.push("");const u=d.filter(t=>t.date>=c);if(u.length>0){e.push(`## Active slot blocks (${u.length} dates)`);const t=[...u].sort((n,a)=>n.date.localeCompare(a.date));for(const n of t.slice(0,20)){const a=o.find(w=>w.id===n.department_id),m=a?k(a.name,a.name_ar,s):n.department_id;e.push(`- ${n.date} — ${m}: ${n.blocked_slots.length} slots blocked (${n.blocked_slots.slice(0,8).join(", ")}${n.blocked_slots.length>8?"…":""})`)}t.length>20&&e.push(`- …and ${t.length-20} more.`),e.push("")}return e.join(`
`)}function O(o){const r=o.getFullYear(),h=String(o.getMonth()+1).padStart(2,"0"),d=String(o.getDate()).padStart(2,"0");return`${r}-${h}-${d}`}const le=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],ce=["الأحد","الإثنين","الثلاثاء","الأربعاء","الخميس","الجمعة","السبت"];function de(o,r){const[h,d,s]=o.split("-").map(g=>parseInt(g,10)),e=new Date(h,d-1,s),l=new Date;l.setHours(0,0,0,0);const c=Math.round((e.getTime()-l.getTime())/864e5),y=(r==="ar"?ce:le)[e.getDay()],f=e.toLocaleDateString(r==="ar"?"ar-EG":void 0,{day:"numeric",month:"short"}),p=`${y} ${f}`;return c===0?`Today (${p})`:c===1?`Tomorrow (${p})`:p}function D(o,r,h,d){const s=(o.working_hours??L)[j(r)];if(!s.open)return{totalSlots:0,booked:0,blocked:0,free:0,freeSlots:[]};const e=U(s).filter(u=>!q(u,s)),l=X(h,r,o.id),c=new Set(d.filter(u=>u.department_id===o.id&&u.date===r).flatMap(u=>u.blocked_slots)),y=new Date,f=O(y),p=y.getHours()*60+y.getMinutes(),g=r===f,b=[];let x=0,S=0;for(const u of e){if(l.has(u)){x++;continue}if(c.has(u)){S++;continue}g&&H(u)<p||b.push(u)}return{totalSlots:e.length,booked:x,blocked:S,free:b.length,freeSlots:b}}function he(o,r,h,d){let s=0,e=0,l=0,c=0;for(const y of r){const f=D(y,o,h,d);s+=f.totalSlots,e+=f.booked,l+=f.blocked,c+=f.free}return{totalSlots:s,booked:e,blocked:l,free:c}}function Se({heading:o,description:r,storageKey:h,defaultText:d,showLivePreview:s=!0}){const{t:e,lang:l}=F(),{value:c,set:y,reset:f}=se(h,d),[p,g]=v.useState(c);v.useEffect(()=>{g(c)},[c]);const b=p!==c,{items:x}=N("departments",V),{items:S}=N("providers",W),{items:u}=N("appointments",K),{items:t}=N("slot_overrides",G),n=v.useMemo(()=>re({clinics:x,providers:S,appointments:u,overrides:t,lang:l}),[x,S,u,t,l]),a=`${c.trim()}

${n}`.trim(),[m,w]=v.useState(!1),[_,E]=v.useState("idle"),M=async()=>{try{await navigator.clipboard.writeText(a),w(!0),setTimeout(()=>w(!1),1500)}catch{}},P=async()=>{E("sending");try{const $=await fetch("/api/demo/clinic/agent/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(h==="persona"?{persona:c}:h==="kb"?{kb:c}:{})});if(!$.ok)throw new Error(`HTTP ${$.status}`);E("ok"),setTimeout(()=>E("idle"),1500)}catch(A){console.error("Apply to agent failed:",A),E("err"),setTimeout(()=>E("idle"),2500)}};return i.jsxs("div",{className:"space-y-6",children:[i.jsxs("div",{className:"flex flex-wrap items-end justify-between gap-3",children:[i.jsxs("div",{children:[i.jsx("h1",{className:"text-2xl font-semibold tracking-tight text-foreground",children:o}),i.jsx("p",{className:"mt-1 max-w-3xl text-sm text-muted-foreground",children:r})]}),i.jsxs("div",{className:"flex items-center gap-2",children:[b?i.jsxs("span",{className:"text-xs text-amber-600 dark:text-amber-400",children:["● ",e("unsavedChanges")]}):i.jsx("span",{className:"text-xs text-muted-foreground",children:e("saved")}),i.jsxs(T,{variant:"outline",onClick:f,children:[i.jsx(z,{className:"me-2 h-4 w-4"}),e("resetToDefault")]}),i.jsxs(T,{onClick:()=>y(p),disabled:!b,children:[i.jsx(Q,{className:"me-2 h-4 w-4"}),e("save")]}),i.jsxs(T,{variant:"outline",onClick:P,disabled:_==="sending"||b,title:e(b?"unsavedChanges":"applyToAgent"),children:[i.jsx(oe,{className:"me-2 h-4 w-4"}),e(_==="ok"?"applied":_==="err"?"applyFailed":"applyToAgent")]})]})]}),i.jsx(C,{title:e("editableSection"),meta:`${p.length.toLocaleString()} chars`,children:i.jsx("div",{className:"p-4",children:i.jsx(Y,{value:p,onChange:A=>g(A.target.value),rows:18,dir:"auto",className:"resize-y font-mono text-[12.5px] leading-relaxed"})})}),s&&i.jsx(C,{title:e("liveStatePreview"),meta:`${n.length.toLocaleString()} chars · auto`,children:i.jsx("pre",{className:"max-h-[420px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-muted-foreground",dir:"auto",children:n})}),i.jsx(C,{title:e("compiledPrompt"),meta:`${a.length.toLocaleString()} chars`,headerExtra:i.jsxs(T,{size:"sm",variant:"outline",onClick:A=>{A.stopPropagation(),M()},children:[m?i.jsx(Z,{className:"me-1.5 h-3.5 w-3.5"}):i.jsx(ee,{className:"me-1.5 h-3.5 w-3.5"}),e(m?"copied":"copyPrompt")]}),children:i.jsx("pre",{className:"max-h-[480px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-foreground",dir:"auto",children:a})})]})}function C({title:o,meta:r,headerExtra:h,children:d}){const[s,e]=v.useState(!1);return i.jsxs("div",{className:"overflow-hidden rounded-xl border border-border bg-card",children:[i.jsxs("button",{type:"button",onClick:()=>e(l=>!l),"aria-expanded":s,className:"flex w-full items-center justify-between gap-3 border-b border-border bg-card px-5 py-3 text-start hover:bg-accent/40 transition-colors",children:[i.jsxs("div",{className:"flex items-center gap-2",children:[i.jsx(te,{className:`h-4 w-4 text-muted-foreground transition-transform ${s?"rotate-0":"-rotate-90"}`}),i.jsx("h2",{className:"text-sm font-semibold text-card-foreground",children:o})]}),i.jsxs("div",{className:"flex items-center gap-3",children:[r&&i.jsx("span",{className:"text-xs text-muted-foreground",children:r}),h]})]}),s&&d]})}export{xe as D,Se as P,_e as a};
