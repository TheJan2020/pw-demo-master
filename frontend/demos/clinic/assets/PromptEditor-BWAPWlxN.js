...
`;function ie({clinics:o,providers:r,appointments:h,overrides:d,lang:s}){const e=[],l=new Date,c=D(l),y=U(l);e.push("# Live clinic state (auto-generated — do NOT edit, refreshes per call)"),e.push(`Generated at: ${l.toISOString()}`),e.push(""),e.push(`## Clinics (${o.length} total)`);for(const t of o){const n=t.head_id?r.find(w=>w.id===t.head_id):null,a=(t.working_hours??$)[j(c)],f=a.open?`${a.open_time}–${a.close_time}${a.break_enabled?` (break ${a.break_start}–${a.break_end})`:""}`:"CLOSED today";e.push(`- ${_(t.name,t.name_ar,s)} · ${_(t.location,t.location_ar,s)} · ${_(t.specialty,t.specialty_ar,s)}`+(n?` · head: ${_(n.name,n.name_ar,s)}`:"")+` · today: ${f}`+(t.active?"":" · INACTIVE"))}e.push("");const u=r.filter(t=>t.active),m={};for(const t of u)m[t.role]=m[t.role]?[...m[t.role],t]:[t];e.push(`## Active staff (${u.length} total)`);for(const t of["doctor","nurse","tech","admin"]){const n=m[t]??[];if(n.length!==0){e.push(`- **${t}** (${n.length}):`);for(const a of n)e.push(`  - ${_(a.name,a.name_ar,s)} · ${_(a.specialty,a.specialty_ar,s)} · ${a.phone}`)}}e.push("");const g=ce(c,o,h,d);e.push(`## Today's totals (${c})`),e.push(`Across all clinics — slots ${g.totalSlots} · booked ${g.booked} · blocked ${g.blocked} · free ${g.free}`),e.push("");const b=7,v=6,S=[];{const t=new Date;t.setHours(0,0,0,0);for(let n=0;n<b;n++){const a=new Date(t);a.setDate(t.getDate()+n),S.push(D(a))}}e.push(`## Free slots — today and next ${b-1} days (per clinic)`);for(const t of o){e.push(`### ${_(t.name,t.name_ar,s)} — ${_(t.specialty,t.specialty_ar,s)}`);for(const n of S){const a=C(t,n,h,d),f=le(n,s);if(a.totalSlots===0){e.push(`- ${f}: closed`);continue}if(a.free===0){e.push(`- ${f}: FULL (${a.booked} booked${a.blocked>0?`, ${a.blocked} blocked`:""})`);continue}const w=a.freeSlots.slice(0,v).join(", "),T=a.freeSlots.length>v?` +${a.freeSlots.length-v} more`:"";e.push(`- ${f}: ${w}${T}  (${a.free} of ${a.totalSlots} free)`)}e.push("")}e.push("## This week totals (per clinic)");for(const t of o){let n=0,a=0,f=0;for(const w of y){const T=C(t,w,h,d);f+=T.totalSlots,n+=T.booked,a+=T.blocked}e.push(`- ${_(t.name,t.name_ar,s)}: slots ${f} · booked ${n} · blocked ${a} · free ${Math.max(0,f-n-a)}`)}e.push("");const p=d.filter(t=>t.date>=c);if(p.length>0){e.push(`## Active slot blocks (${p.length} dates)`);const t=[...p].sort((n,a)=>n.date.localeCompare(a.date));for(const n of t.slice(0,20)){const a=o.find(w=>w.id===n.department_id),f=a?_(a.name,a.name_ar,s):n.department_id;e.push(`- ${n.date} — ${f}: ${n.blocked_slots.length} slots blocked (${n.blocked_slots.slice(0,8).join(", ")}${n.blocked_slots.length>8?"…":""})`)}t.length>20&&e.push(`- …and ${t.length-20} more.`),e.push("")}return e.join(`
`)}function D(o){const r=o.getFullYear(),h=String(o.getMonth()+1).padStart(2,"0"),d=String(o.getDate()).padStart(2,"0");return`${r}-${h}-${d}`}const se=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],re=["الأحد","الإثنين","الثلاثاء","الأربعاء","الخميس","الجمعة","السبت"];function le(o,r){const[h,d,s]=o.split("-").map(g=>parseInt(g,10)),e=new Date(h,d-1,s),l=new Date;l.setHours(0,0,0,0);const c=Math.round((e.getTime()-l.getTime())/864e5),y=(r==="ar"?re:se)[e.getDay()],u=e.toLocaleDateString(r==="ar"?"ar-EG":void 0,{day:"numeric",month:"short"}),m=`${y} ${u}`;return c===0?`Today (${m})`:c===1?`Tomorrow (${m})`:m}function C(o,r,h,d){const s=(o.working_hours??$)[j(r)];if(!s.open)return{totalSlots:0,booked:0,blocked:0,free:0,freeSlots:[]};const e=B(s).filter(p=>!H(p,s)),l=W(h,r,o.id),c=new Set(d.filter(p=>p.department_id===o.id&&p.date===r).flatMap(p=>p.blocked_slots)),y=new Date,u=D(y),m=y.getHours()*60+y.getMinutes(),g=r===u,b=[];let v=0,S=0;for(const p of e){if(l.has(p)){v++;continue}if(c.has(p)){S++;continue}g&&q(p)<m||b.push(p)}return{totalSlots:e.length,booked:v,blocked:S,free:b.length,freeSlots:b}}function ce(o,r,h,d){let s=0,e=0,l=0,c=0;for(const y of r){const u=C(y,o,h,d);s+=u.totalSlots,e+=u.booked,l+=u.blocked,c+=u.free}return{totalSlots:s,booked:e,blocked:l,free:c}}function ve({heading:o,description:r,storageKey:h,defaultText:d,showLivePreview:s=!0}){const{t:e,lang:l}=Y(),{value:c,set:y,reset:u}=ne(h,d),[m,g]=k.useState(c);k.useEffect(()=>{g(c)},[c]);const b=m!==c,{items:v}=E("departments",V),{items:S}=E("providers",G),{items:p}=E("appointments",X),{items:t}=E("slot_overrides",K),n=k.useMemo(()=>ie({clinics:v,providers:S,appointments:p,overrides:t,lang:l}),[v,S,p,t,l]),a=`${c.trim()}

${n}`.trim(),[f,w]=k.useState(!1),[T,N]=k.useState("idle"),M=async()=>{try{await navigator.clipboard.writeText(a),w(!0),setTimeout(()=>w(!1),1500)}catch{}},P=async()=>{N("sending");try{const L=await fetch("/api/demo/clinic/agent/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(h==="persona"?{persona:c}:h==="kb"?{kb:c}:{})});if(!L.ok)throw new Error(`HTTP ${L.status}`);N("ok"),setTimeout(()=>N("idle"),1500)}catch(A){console.error("Apply to agent failed:",A),N("err"),setTimeout(()=>N("idle"),2500)}};return i.jsxs("div",{className:"space-y-6",children:[i.jsxs("div",{className:"flex flex-wrap items-end justify-between gap-3",children:[i.jsxs("div",{children:[i.jsx("h1",{className:"text-2xl font-semibold tracking-tight text-foreground",children:o}),i.jsx("p",{className:"mt-1 max-w-3xl text-sm text-muted-foreground",children:r})]}),i.jsxs("div",{className:"flex items-center gap-2",children:[b?i.jsxs("span",{className:"text-xs text-amber-600 dark:text-amber-400",children:["● ",e("unsavedChanges")]}):i.jsx("span",{className:"text-xs text-muted-foreground",children:e("saved")}),i.jsxs(x,{variant:"outline",onClick:u,children:[i.jsx(z,{className:"me-2 h-4 w-4"}),e("resetToDefault")]}),i.jsxs(x,{onClick:()=>y(m),disabled:!b,children:[i.jsx(Q,{className:"me-2 h-4 w-4"}),e("save")]}),i.jsxs(x,{variant:"outline",onClick:P,disabled:T==="sending"||b,title:e(b?"unsavedChanges":"applyToAgent"),children:[i.jsx(J,{className:"me-2 h-4 w-4"}),e(T==="ok"?"applied":T==="err"?"applyFailed":"applyToAgent")]})]})]}),i.jsx(R,{title:e("editableSection"),meta:`${m.length.toLocaleString()} chars`,children:i.jsx("div",{className:"p-4",children:i.jsx(F,{value:m,onChange:A=>g(A.target.value),rows:18,dir:"auto",className:"resize-y font-mono text-[12.5px] leading-relaxed"})})}),s&&i.jsx(R,{title:e("liveStatePreview"),meta:`${n.length.toLocaleString()} chars · auto`,children:i.jsx("pre",{className:"max-h-[420px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-muted-foreground",dir:"auto",children:n})}),i.jsx(R,{title:e("compiledPrompt"),meta:`${a.length.toLocaleString()} chars`,headerExtra:i.jsxs(x,{size:"sm",variant:"outline",onClick:A=>{A.stopPropagation(),M()},children:[f?i.jsx(Z,{className:"me-1.5 h-3.5 w-3.5"}):i.jsx(ee,{className:"me-1.5 h-3.5 w-3.5"}),e(f?"copied":"copyPrompt")]}),children:i.jsx("pre",{className:"max-h-[480px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-foreground",dir:"auto",children:a})})]})}function R({title:o,meta:r,headerExtra:h,children:d}){const[s,e]=k.useState(!1);return i.jsxs("div",{className:"overflow-hidden rounded-xl border border-border bg-card",children:[i.jsxs("button",{type:"button",onClick:()=>e(l=>!l),"aria-expanded":s,className:"flex w-full items-center justify-between gap-3 border-b border-border bg-card px-5 py-3 text-start hover:bg-accent/40 transition-colors",children:[i.jsxs("div",{className:"flex items-center gap-2",children:[i.jsx(te,{className:`h-4 w-4 text-muted-foreground transition-transform ${s?"rotate-0":"-rotate-90"}`}),i.jsx("h2",{className:"text-sm font-semibold text-card-foreground",children:o})]}),i.jsxs("div",{className:"flex items-center gap-3",children:[r&&i.jsx("span",{className:"text-xs text-muted-foreground",children:r}),h]})]}),s&&d]})}export{_e as D,ve as P,ke as a};
