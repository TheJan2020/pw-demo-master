import{c as _,u as P,j as i}from"./index-BwxFsOdx.js";import{B as N}from"./button-CCvWdOEO.js";import{T as F}from"./textarea-momC2uqT.js";import{w as U,D as $,p as M,l as w,s as B,i as H,f as W,t as q,u as x,a as V,c as G,g as X,d as K}from"./demoStore-DkRfZ1C8.js";import{R as z}from"./rotate-ccw-B9Pn9uIT.js";import{S as Q}from"./save-Dln_IWMD.js";import{S as J}from"./send-DV_vKpVC.js";import{C as Z}from"./check-CwYhgfH7.js";import{C as ee}from"./copy-BwWmFnOn.js";import{C as te}from"./chevron-down-DBgzHOXK.js";const ae="pwdemo:clinic:doc",oe=1,I="pwdemo:clinic:doc-change";function O(o){return`${ae}:${o}:v${oe}`}function ne(o,r){const[h,d]=_.useState(()=>{if(typeof window>"u")return r;try{return localStorage.getItem(O(o))??r}catch{return r}});_.useEffect(()=>{if(typeof window>"u")return;const l=c=>{if(c.detail?.key===o)try{const u=localStorage.getItem(O(o));u!==null&&d(u)}catch{}};return window.addEventListener(I,l),()=>window.removeEventListener(I,l)},[o]);const s=_.useCallback(l=>{d(l);try{localStorage.setItem(O(o),l)}catch{}window.dispatchEvent(new CustomEvent(I,{detail:{key:o}}))},[o]),e=_.useCallback(()=>{d(r);try{localStorage.removeItem(O(o))}catch{}window.dispatchEvent(new CustomEvent(I,{detail:{key:o}}))},[o,r]);return{value:h,set:s,reset:e}}const we=`# Primewave Mate Clinics — Riyadh
...
`,_e=`# Layla — Receptionist persona for Primewave Mate Clinics
...
`;function ie({clinics:o,providers:r,appointments:h,overrides:d,lang:s}){const e=[],l=new Date,c=R(l),y=U(l);e.push("# Live clinic state (auto-generated — do NOT edit, refreshes per call)"),e.push(`Generated at: ${l.toISOString()}`),e.push(""),e.push(`## Clinics (${o.length} total)`);for(const t of o){const n=t.head_id?r.find(k=>k.id===t.head_id):null,a=(t.working_hours??$)[M(c)],f=a.open?`${a.open_time}–${a.close_time}${a.break_enabled?` (break ${a.break_start}–${a.break_end})`:""}`:"CLOSED today";e.push(`- ${w(t.name,t.name_ar,s)} · ${w(t.location,t.location_ar,s)} · ${w(t.specialty,t.specialty_ar,s)}`+(n?` · head: ${w(n.name,n.name_ar,s)}`:"")+` · today: ${f}`+(t.active?"":" · INACTIVE"))}e.push("");const u=r.filter(t=>t.active),m={};for(const t of u)m[t.role]=m[t.role]?[...m[t.role],t]:[t];e.push(`## Active staff (${u.length} total)`);for(const t of["doctor","nurse","tech","admin"]){const n=m[t]??[];if(n.length!==0){e.push(`- **${t}** (${n.length}):`);for(const a of n)e.push(`  - ${w(a.name,a.name_ar,s)} · ${w(a.specialty,a.specialty_ar,s)} · ${a.phone}`)}}e.push("");const g=ce(c,o,h,d);e.push(`## Today's totals (${c})`),e.push(`Across all clinics — slots ${g.totalSlots} · booked ${g.booked} · blocked ${g.blocked} · free ${g.free}`),e.push("");const b=7,v=6,A=[];{const t=new Date;t.setHours(0,0,0,0);for(let n=0;n<b;n++){const a=new Date(t);a.setDate(t.getDate()+n),A.push(R(a))}}e.push(`## Free slots — today and next ${b-1} days (per clinic)`);for(const t of o){e.push(`### ${w(t.name,t.name_ar,s)} — ${w(t.specialty,t.specialty_ar,s)}`);for(const n of A){const a=C(t,n,h,d),f=le(n,s);if(a.totalSlots===0){e.push(`- ${f}: closed`);continue}if(a.free===0){e.push(`- ${f}: FULL (${a.booked} booked${a.blocked>0?`, ${a.blocked} blocked`:""})`);continue}const k=a.freeSlots.slice(0,v).join(", "),T=a.freeSlots.length>v?` +${a.freeSlots.length-v} more`:"";e.push(`- ${f}: ${k}${T}  (${a.free} of ${a.totalSlots} free)`)}e.push("")}e.push("## This week totals (per clinic)");for(const t of o){let n=0,a=0,f=0;for(const k of y){const T=C(t,k,h,d);f+=T.totalSlots,n+=T.booked,a+=T.blocked}e.push(`- ${w(t.name,t.name_ar,s)}: slots ${f} · booked ${n} · blocked ${a} · free ${Math.max(0,f-n-a)}`)}e.push("");const p=d.filter(t=>t.date>=c);if(p.length>0){e.push(`## Active slot blocks (${p.length} dates)`);const t=[...p].sort((n,a)=>n.date.localeCompare(a.date));for(const n of t.slice(0,20)){const a=o.find(k=>k.id===n.department_id),f=a?w(a.name,a.name_ar,s):n.department_id;e.push(`- ${n.date} — ${f}: ${n.blocked_slots.length} slots blocked (${n.blocked_slots.slice(0,8).join(", ")}${n.blocked_slots.length>8?"…":""})`)}t.length>20&&e.push(`- …and ${t.length-20} more.`),e.push("")}return e.join(`
`)}function R(o){const r=o.getFullYear(),h=String(o.getMonth()+1).padStart(2,"0"),d=String(o.getDate()).padStart(2,"0");return`${r}-${h}-${d}`}const se=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],re=["الأحد","الإثنين","الثلاثاء","الأربعاء","الخميس","الجمعة","السبت"];function le(o,r){const[h,d,s]=o.split("-").map(g=>parseInt(g,10)),e=new Date(h,d-1,s),l=new Date;l.setHours(0,0,0,0);const c=Math.round((e.getTime()-l.getTime())/864e5),y=(r==="ar"?re:se)[e.getDay()],u=e.toLocaleDateString(r==="ar"?"ar-EG":void 0,{day:"numeric",month:"short"}),m=`${y} ${u}`;return c===0?`Today (${m})`:c===1?`Tomorrow (${m})`:m}function C(o,r,h,d){const s=(o.working_hours??$)[M(r)];if(!s.open)return{totalSlots:0,booked:0,blocked:0,free:0,freeSlots:[]};const e=B(s).filter(p=>!H(p,s)),l=W(h,r,o.id),c=new Set(d.filter(p=>p.department_id===o.id&&p.date===r).flatMap(p=>p.blocked_slots)),y=new Date,u=R(y),m=y.getHours()*60+y.getMinutes(),g=r===u,b=[];let v=0,A=0;for(const p of e){if(l.has(p)){v++;continue}if(c.has(p)){A++;continue}g&&q(p)<m||b.push(p)}return{totalSlots:e.length,booked:v,blocked:A,free:b.length,freeSlots:b}}function ce(o,r,h,d){let s=0,e=0,l=0,c=0;for(const y of r){const u=C(y,o,h,d);s+=u.totalSlots,e+=u.booked,l+=u.blocked,c+=u.free}return{totalSlots:s,booked:e,blocked:l,free:c}}function ve({heading:o,description:r,storageKey:h,defaultText:d,showLivePreview:s=!0}){const{t:e,lang:l}=P(),{value:c,set:y,reset:u}=ne(h,d),[m,g]=_.useState(c);_.useEffect(()=>{g(c)},[c]);const b=m!==c,{items:v}=x("departments",V),{items:A}=x("providers",G),{items:p}=x("appointments",X),{items:t}=x("slot_overrides",K),n=_.useMemo(()=>ie({clinics:v,providers:A,appointments:p,overrides:t,lang:l}),[v,A,p,t,l]),a=`${c.trim()}

${n}`.trim(),[f,k]=_.useState(!1),[T,E]=_.useState("idle"),j=async()=>{try{await navigator.clipboard.writeText(a),k(!0),setTimeout(()=>k(!1),1500)}catch{}},Y=async()=>{E("sending");try{const L=await fetch("/api/demo/clinic/agent/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(h==="persona"?{persona:c}:h==="kb"?{kb:c}:{})});if(!L.ok)throw new Error(`HTTP ${L.status}`);E("ok"),setTimeout(()=>E("idle"),1500)}catch(S){console.error("Apply to agent failed:",S),E("err"),setTimeout(()=>E("idle"),2500)}};return i.jsxs("div",{className:"space-y-6",children:[i.jsxs("div",{className:"flex flex-wrap items-end justify-between gap-3",children:[i.jsxs("div",{children:[i.jsx("h1",{className:"text-2xl font-semibold tracking-tight text-foreground",children:o}),i.jsx("p",{className:"mt-1 max-w-3xl text-sm text-muted-foreground",children:r})]}),i.jsxs("div",{className:"flex items-center gap-2",children:[b?i.jsxs("span",{className:"text-xs text-amber-600 dark:text-amber-400",children:["● ",e("unsavedChanges")]}):i.jsx("span",{className:"text-xs text-muted-foreground",children:e("saved")}),i.jsxs(N,{variant:"outline",onClick:u,children:[i.jsx(z,{className:"me-2 h-4 w-4"}),e("resetToDefault")]}),i.jsxs(N,{onClick:()=>y(m),disabled:!b,children:[i.jsx(Q,{className:"me-2 h-4 w-4"}),e("save")]}),i.jsxs(N,{variant:"outline",onClick:Y,disabled:T==="sending"||b,title:e(b?"unsavedChanges":"applyToAgent"),children:[i.jsx(J,{className:"me-2 h-4 w-4"}),e(T==="ok"?"applied":T==="err"?"applyFailed":"applyToAgent")]})]})]}),i.jsx(D,{title:e("editableSection"),meta:`${m.length.toLocaleString()} chars`,children:i.jsx("div",{className:"p-4",children:i.jsx(F,{value:m,onChange:S=>g(S.target.value),rows:18,dir:"auto",className:"resize-y font-mono text-[12.5px] leading-relaxed"})})}),s&&i.jsx(D,{title:e("liveStatePreview"),meta:`${n.length.toLocaleString()} chars · auto`,children:i.jsx("pre",{className:"max-h-[420px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-muted-foreground",dir:"auto",children:n})}),i.jsx(D,{title:e("compiledPrompt"),meta:`${a.length.toLocaleString()} chars`,headerExtra:i.jsxs(N,{size:"sm",variant:"outline",onClick:S=>{S.stopPropagation(),j()},children:[f?i.jsx(Z,{className:"me-1.5 h-3.5 w-3.5"}):i.jsx(ee,{className:"me-1.5 h-3.5 w-3.5"}),e(f?"copied":"copyPrompt")]}),children:i.jsx("pre",{className:"max-h-[480px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-foreground",dir:"auto",children:a})})]})}function D({title:o,meta:r,headerExtra:h,children:d}){const[s,e]=_.useState(!1);return i.jsxs("div",{className:"overflow-hidden rounded-xl border border-border bg-card",children:[i.jsxs("button",{type:"button",onClick:()=>e(l=>!l),"aria-expanded":s,className:"flex w-full items-center justify-between gap-3 border-b border-border bg-card px-5 py-3 text-start hover:bg-accent/40 transition-colors",children:[i.jsxs("div",{className:"flex items-center gap-2",children:[i.jsx(te,{className:`h-4 w-4 text-muted-foreground transition-transform ${s?"rotate-0":"-rotate-90"}`}),i.jsx("h2",{className:"text-sm font-semibold text-card-foreground",children:o})]}),i.jsxs("div",{className:"flex items-center gap-3",children:[r&&i.jsx("span",{className:"text-xs text-muted-foreground",children:r}),h]})]}),s&&d]})}export{we as D,ve as P,_e as a};
