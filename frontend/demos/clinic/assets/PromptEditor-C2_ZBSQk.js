import{c as k,u as P,j as n}from"./index-CYKlA3a4.js";import{B as D}from"./button-BxYDIppL.js";import{T as I}from"./textarea-BEm2AT32.js";import{w as L,D as R,j as M,l as w,s as O,i as B,e as X,t as Y,u as A,S as F,b as H,g as U,c as G,C as W}from"./demoStore-C8F4XAZI.js";import{R as z}from"./rotate-ccw-0RoL9oBL.js";import{c as K}from"./createLucideIcon-nX3OmMCs.js";import{C as V}from"./check-DtwNL8zD.js";import{C as q}from"./copy-uTiLbE8u.js";const Q=[["path",{d:"M15.2 3a2 2 0 0 1 1.4.6l3.8 3.8a2 2 0 0 1 .6 1.4V19a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2z",key:"1c8476"}],["path",{d:"M17 21v-7a1 1 0 0 0-1-1H8a1 1 0 0 0-1 1v7",key:"1ydtos"}],["path",{d:"M7 3v4a1 1 0 0 0 1 1h7",key:"t51u73"}]],Z=K("save",Q),J="pwdemo:clinic:doc",ee=1,E="pwdemo:clinic:doc-change";function _(a){return`${J}:${a}:v${ee}`}function te(a,r){const[h,c]=k.useState(()=>{if(typeof window>"u")return r;try{return localStorage.getItem(_(a))??r}catch{return r}});k.useEffect(()=>{if(typeof window>"u")return;const l=d=>{if(d.detail?.key===a)try{const m=localStorage.getItem(_(a));m!==null&&c(m)}catch{}};return window.addEventListener(E,l),()=>window.removeEventListener(E,l)},[a]);const i=k.useCallback(l=>{c(l);try{localStorage.setItem(_(a),l)}catch{}window.dispatchEvent(new CustomEvent(E,{detail:{key:a}}))},[a]),e=k.useCallback(()=>{c(r);try{localStorage.removeItem(_(a))}catch{}window.dispatchEvent(new CustomEvent(E,{detail:{key:a}}))},[a,r]);return{value:h,set:i,reset:e}}const fe=`# Primewave Mate Clinics — Riyadh
...
`,ge=`# Layla — Receptionist persona for Primewave Mate Clinics
...
`;function oe({clinics:a,providers:r,appointments:h,overrides:c,lang:i}){const e=[],l=new Date,d=j(l),g=L(l);e.push("# Live clinic state (auto-generated — do NOT edit, refreshes per call)"),e.push(`Generated at: ${l.toISOString()}`),e.push(""),e.push(`## Clinics (${a.length} total)`);for(const t of a){const s=t.head_id?r.find(y=>y.id===t.head_id):null,o=(t.working_hours??R)[M(d)],f=o.open?`${o.open_time}–${o.close_time}${o.break_enabled?` (break ${o.break_start}–${o.break_end})`:""}`:"CLOSED today";e.push(`- ${w(t.name,t.name_ar,i)} · ${w(t.location,t.location_ar,i)} · ${w(t.specialty,t.specialty_ar,i)}`+(s?` · head: ${w(s.name,s.name_ar,i)}`:"")+` · today: ${f}`+(t.active?"":" · INACTIVE"))}e.push("");const m=r.filter(t=>t.active),p={};for(const t of m)p[t.role]=p[t.role]?[...p[t.role],t]:[t];e.push(`## Active staff (${m.length} total)`);for(const t of["doctor","nurse","tech","admin"]){const s=p[t]??[];if(s.length!==0){e.push(`- **${t}** (${s.length}):`);for(const o of s)e.push(`  - ${w(o.name,o.name_ar,i)} · ${w(o.specialty,o.specialty_ar,i)} · ${o.phone}`)}}e.push("");const b=ie(d,a,h,c);e.push(`## Today's totals (${d})`),e.push(`Across all clinics — slots ${b.totalSlots} · booked ${b.booked} · blocked ${b.blocked} · free ${b.free}`),e.push("");const x=7,v=6,S=[];{const t=new Date;t.setHours(0,0,0,0);for(let s=0;s<x;s++){const o=new Date(t);o.setDate(t.getDate()+s),S.push(j(o))}}e.push(`## Free slots — today and next ${x-1} days (per clinic)`);for(const t of a){e.push(`### ${w(t.name,t.name_ar,i)} — ${w(t.specialty,t.specialty_ar,i)}`);for(const s of S){const o=T(t,s,h,c),f=ne(s,i);if(o.totalSlots===0){e.push(`- ${f}: closed`);continue}if(o.free===0){e.push(`- ${f}: FULL (${o.booked} booked${o.blocked>0?`, ${o.blocked} blocked`:""})`);continue}const y=o.freeSlots.slice(0,v).join(", "),$=o.freeSlots.length>v?` +${o.freeSlots.length-v} more`:"";e.push(`- ${f}: ${y}${$}  (${o.free} of ${o.totalSlots} free)`)}e.push("")}e.push("## This week totals (per clinic)");for(const t of a){let s=0,o=0,f=0;for(const y of g){const $=T(t,y,h,c);f+=$.totalSlots,s+=$.booked,o+=$.blocked}e.push(`- ${w(t.name,t.name_ar,i)}: slots ${f} · booked ${s} · blocked ${o} · free ${Math.max(0,f-s-o)}`)}e.push("");const u=c.filter(t=>t.date>=d);if(u.length>0){e.push(`## Active slot blocks (${u.length} dates)`);const t=[...u].sort((s,o)=>s.date.localeCompare(o.date));for(const s of t.slice(0,20)){const o=a.find(y=>y.id===s.department_id),f=o?w(o.name,o.name_ar,i):s.department_id;e.push(`- ${s.date} — ${f}: ${s.blocked_slots.length} slots blocked (${s.blocked_slots.slice(0,8).join(", ")}${s.blocked_slots.length>8?"…":""})`)}t.length>20&&e.push(`- …and ${t.length-20} more.`),e.push("")}return e.join(`
`)}function j(a){const r=a.getFullYear(),h=String(a.getMonth()+1).padStart(2,"0"),c=String(a.getDate()).padStart(2,"0");return`${r}-${h}-${c}`}const ae=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],se=["الأحد","الإثنين","الثلاثاء","الأربعاء","الخميس","الجمعة","السبت"];function ne(a,r){const[h,c,i]=a.split("-").map(b=>parseInt(b,10)),e=new Date(h,c-1,i),l=new Date;l.setHours(0,0,0,0);const d=Math.round((e.getTime()-l.getTime())/864e5),g=(r==="ar"?se:ae)[e.getDay()],m=e.toLocaleDateString(r==="ar"?"ar-EG":void 0,{day:"numeric",month:"short"}),p=`${g} ${m}`;return d===0?`Today (${p})`:d===1?`Tomorrow (${p})`:p}function T(a,r,h,c){const i=(a.working_hours??R)[M(r)];if(!i.open)return{totalSlots:0,booked:0,blocked:0,free:0,freeSlots:[]};const e=O(i).filter(u=>!B(u,i)),l=X(h,r,a.id),d=new Set(c.filter(u=>u.department_id===a.id&&u.date===r).flatMap(u=>u.blocked_slots)),g=new Date,m=j(g),p=g.getHours()*60+g.getMinutes(),b=r===m,x=[];let v=0,S=0;for(const u of e){if(l.has(u)){v++;continue}if(d.has(u)){S++;continue}b&&Y(u)<p||x.push(u)}return{totalSlots:e.length,booked:v,blocked:S,free:x.length,freeSlots:x}}function ie(a,r,h,c){let i=0,e=0,l=0,d=0;for(const g of r){const m=T(g,a,h,c);i+=m.totalSlots,e+=m.booked,l+=m.blocked,d+=m.free}return{totalSlots:i,booked:e,blocked:l,free:d}}function be({heading:a,description:r,storageKey:h,defaultText:c,showLivePreview:i=!0}){const{t:e,lang:l}=P(),{value:d,set:g,reset:m}=te(h,c),[p,b]=k.useState(d);k.useEffect(()=>{b(d)},[d]);const x=p!==d,{items:v}=A("departments",F),{items:S}=A("providers",H),{items:u}=A("appointments",U),{items:t}=A("slot_overrides",G),s=k.useMemo(()=>oe({clinics:v,providers:S,appointments:u,overrides:t,lang:l}),[v,S,u,t,l]),o=`${d.trim()}

${s}`.trim(),[f,y]=k.useState(!1),$=async()=>{try{await navigator.clipboard.writeText(o),y(!0),setTimeout(()=>y(!1),1500)}catch{}};return n.jsxs("div",{className:"space-y-6",children:[n.jsxs("div",{className:"flex flex-wrap items-end justify-between gap-3",children:[n.jsxs("div",{children:[n.jsx("h1",{className:"text-2xl font-semibold tracking-tight text-foreground",children:a}),n.jsx("p",{className:"mt-1 max-w-3xl text-sm text-muted-foreground",children:r})]}),n.jsxs("div",{className:"flex items-center gap-2",children:[x?n.jsxs("span",{className:"text-xs text-amber-600 dark:text-amber-400",children:["● ",e("unsavedChanges")]}):n.jsx("span",{className:"text-xs text-muted-foreground",children:e("saved")}),n.jsxs(D,{variant:"outline",onClick:m,children:[n.jsx(z,{className:"me-2 h-4 w-4"}),e("resetToDefault")]}),n.jsxs(D,{onClick:()=>g(p),disabled:!x,children:[n.jsx(Z,{className:"me-2 h-4 w-4"}),e("save")]})]})]}),n.jsx(N,{title:e("editableSection"),meta:`${p.length.toLocaleString()} chars`,children:n.jsx("div",{className:"p-4",children:n.jsx(I,{value:p,onChange:C=>b(C.target.value),rows:18,dir:"auto",className:"resize-y font-mono text-[12.5px] leading-relaxed"})})}),i&&n.jsx(N,{title:e("liveStatePreview"),meta:`${s.length.toLocaleString()} chars · auto`,children:n.jsx("pre",{className:"max-h-[420px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-muted-foreground",dir:"auto",children:s})}),n.jsx(N,{title:e("compiledPrompt"),meta:`${o.length.toLocaleString()} chars`,headerExtra:n.jsxs(D,{size:"sm",variant:"outline",onClick:C=>{C.stopPropagation(),$()},children:[f?n.jsx(V,{className:"me-1.5 h-3.5 w-3.5"}):n.jsx(q,{className:"me-1.5 h-3.5 w-3.5"}),e(f?"copied":"copyPrompt")]}),children:n.jsx("pre",{className:"max-h-[480px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-foreground",dir:"auto",children:o})})]})}function N({title:a,meta:r,headerExtra:h,children:c}){const[i,e]=k.useState(!1);return n.jsxs("div",{className:"overflow-hidden rounded-xl border border-border bg-card",children:[n.jsxs("button",{type:"button",onClick:()=>e(l=>!l),"aria-expanded":i,className:"flex w-full items-center justify-between gap-3 border-b border-border bg-card px-5 py-3 text-start hover:bg-accent/40 transition-colors",children:[n.jsxs("div",{className:"flex items-center gap-2",children:[n.jsx(W,{className:`h-4 w-4 text-muted-foreground transition-transform ${i?"rotate-0":"-rotate-90"}`}),n.jsx("h2",{className:"text-sm font-semibold text-card-foreground",children:a})]}),n.jsxs("div",{className:"flex items-center gap-3",children:[r&&n.jsx("span",{className:"text-xs text-muted-foreground",children:r}),h]})]}),i&&c]})}export{fe as D,be as P,ge as a};
