import{c as x,u as X,j as s}from"./index-DsszsdZ4.js";import{B as T}from"./button-DGHB2MjG.js";import{T as F}from"./textarea-BluRWvs9.js";import{w as B,D as O,p as L,l as k,s as Y,i as U,f as H,t as q,u as E,a as W,c as G,g as z,d as V}from"./demoStore-Bx0vIPWl.js";import{R as K}from"./rotate-ccw-DNjqe8zA.js";import{S as Q}from"./save-C7iRq4TO.js";import{c as J}from"./createLucideIcon-yzY-auSO.js";import{C as Z}from"./check-DZjjTkmT.js";import{C as ee}from"./copy-iDvpXt9q.js";import{C as te}from"./chevron-down-BoQKl2yq.js";const ae=[["path",{d:"M14.536 21.686a.5.5 0 0 0 .937-.024l6.5-19a.496.496 0 0 0-.635-.635l-19 6.5a.5.5 0 0 0-.024.937l7.93 3.18a2 2 0 0 1 1.112 1.11z",key:"1ffxy3"}],["path",{d:"m21.854 2.147-10.94 10.939",key:"12cjpa"}]],oe=J("send",ae),ne="pwdemo:clinic:doc",se=1,C="pwdemo:clinic:doc-change";function N(o){return`${ne}:${o}:v${se}`}function ie(o,r){const[h,d]=x.useState(()=>{if(typeof window>"u")return r;try{return localStorage.getItem(N(o))??r}catch{return r}});x.useEffect(()=>{if(typeof window>"u")return;const l=c=>{if(c.detail?.key===o)try{const m=localStorage.getItem(N(o));m!==null&&d(m)}catch{}};return window.addEventListener(C,l),()=>window.removeEventListener(C,l)},[o]);const i=x.useCallback(l=>{d(l);try{localStorage.setItem(N(o),l)}catch{}window.dispatchEvent(new CustomEvent(C,{detail:{key:o}}))},[o]),e=x.useCallback(()=>{d(r);try{localStorage.removeItem(N(o))}catch{}window.dispatchEvent(new CustomEvent(C,{detail:{key:o}}))},[o,r]);return{value:h,set:i,reset:e}}const ve=`# Primewave Mate Clinics — Riyadh
...
`;function re({clinics:o,providers:r,appointments:h,overrides:d,lang:i}){const e=[],l=new Date,c=R(l),y=B(l);e.push("# Live clinic state (auto-generated — do NOT edit, refreshes per call)"),e.push(`Generated at: ${l.toISOString()}`),e.push(""),e.push(`## Clinics (${o.length} total)`);for(const t of o){const n=t.head_id?r.find(w=>w.id===t.head_id):null,a=(t.working_hours??O)[L(c)],p=a.open?`${a.open_time}–${a.close_time}${a.break_enabled?` (break ${a.break_start}–${a.break_end})`:""}`:"CLOSED today";e.push(`- ${k(t.name,t.name_ar,i)} · ${k(t.location,t.location_ar,i)} · ${k(t.specialty,t.specialty_ar,i)}`+(n?` · head: ${k(n.name,n.name_ar,i)}`:"")+` · today: ${p}`+(t.active?"":" · INACTIVE"))}e.push("");const m=r.filter(t=>t.active),f={};for(const t of m)f[t.role]=f[t.role]?[...f[t.role],t]:[t];e.push(`## Active staff (${m.length} total)`);for(const t of["doctor","nurse","tech","admin"]){const n=f[t]??[];if(n.length!==0){e.push(`- **${t}** (${n.length}):`);for(const a of n)e.push(`  - ${k(a.name,a.name_ar,i)} · ${k(a.specialty,a.specialty_ar,i)} · ${a.phone}`)}}e.push("");const g=he(c,o,h,d);e.push(`## Today's totals (${c})`),e.push(`Across all clinics — slots ${g.totalSlots} · booked ${g.booked} · blocked ${g.blocked} · free ${g.free}`),e.push("");const b=7,v=6,A=[];{const t=new Date;t.setHours(0,0,0,0);for(let n=0;n<b;n++){const a=new Date(t);a.setDate(t.getDate()+n),A.push(R(a))}}e.push(`## Free slots — today and next ${b-1} days (per clinic)`);for(const t of o){e.push(`### ${k(t.name,t.name_ar,i)} — ${k(t.specialty,t.specialty_ar,i)}`);for(const n of A){const a=j(t,n,h,d),p=de(n,i);if(a.totalSlots===0){e.push(`- ${p}: closed`);continue}if(a.free===0){e.push(`- ${p}: FULL (${a.booked} booked${a.blocked>0?`, ${a.blocked} blocked`:""})`);continue}const w=a.freeSlots.slice(0,v).join(", "),S=a.freeSlots.length>v?` +${a.freeSlots.length-v} more`:"";e.push(`- ${p}: ${w}${S}  (${a.free} of ${a.totalSlots} free)`)}e.push("")}e.push("## This week totals (per clinic)");for(const t of o){let n=0,a=0,p=0;for(const w of y){const S=j(t,w,h,d);p+=S.totalSlots,n+=S.booked,a+=S.blocked}e.push(`- ${k(t.name,t.name_ar,i)}: slots ${p} · booked ${n} · blocked ${a} · free ${Math.max(0,p-n-a)}`)}e.push("");const u=d.filter(t=>t.date>=c);if(u.length>0){e.push(`## Active slot blocks (${u.length} dates)`);const t=[...u].sort((n,a)=>n.date.localeCompare(a.date));for(const n of t.slice(0,20)){const a=o.find(w=>w.id===n.department_id),p=a?k(a.name,a.name_ar,i):n.department_id;e.push(`- ${n.date} — ${p}: ${n.blocked_slots.length} slots blocked (${n.blocked_slots.slice(0,8).join(", ")}${n.blocked_slots.length>8?"…":""})`)}t.length>20&&e.push(`- …and ${t.length-20} more.`),e.push("")}return e.join(`
`)}function R(o){const r=o.getFullYear(),h=String(o.getMonth()+1).padStart(2,"0"),d=String(o.getDate()).padStart(2,"0");return`${r}-${h}-${d}`}const le=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],ce=["الأحد","الإثنين","الثلاثاء","الأربعاء","الخميس","الجمعة","السبت"];function de(o,r){const[h,d,i]=o.split("-").map(g=>parseInt(g,10)),e=new Date(h,d-1,i),l=new Date;l.setHours(0,0,0,0);const c=Math.round((e.getTime()-l.getTime())/864e5),y=(r==="ar"?ce:le)[e.getDay()],m=e.toLocaleDateString(r==="ar"?"ar-EG":void 0,{day:"numeric",month:"short"}),f=`${y} ${m}`;return c===0?`Today (${f})`:c===1?`Tomorrow (${f})`:f}function j(o,r,h,d){const i=(o.working_hours??O)[L(r)];if(!i.open)return{totalSlots:0,booked:0,blocked:0,free:0,freeSlots:[]};const e=Y(i).filter(u=>!U(u,i)),l=H(h,r,o.id),c=new Set(d.filter(u=>u.department_id===o.id&&u.date===r).flatMap(u=>u.blocked_slots)),y=new Date,m=R(y),f=y.getHours()*60+y.getMinutes(),g=r===m,b=[];let v=0,A=0;for(const u of e){if(l.has(u)){v++;continue}if(c.has(u)){A++;continue}g&&q(u)<f||b.push(u)}return{totalSlots:e.length,booked:v,blocked:A,free:b.length,freeSlots:b}}function he(o,r,h,d){let i=0,e=0,l=0,c=0;for(const y of r){const m=j(y,o,h,d);i+=m.totalSlots,e+=m.booked,l+=m.blocked,c+=m.free}return{totalSlots:i,booked:e,blocked:l,free:c}}function Ae({heading:o,description:r,storageKey:h,defaultText:d,showLivePreview:i=!0}){const{t:e,lang:l}=X(),{value:c,set:y,reset:m}=ie(h,d),[f,g]=x.useState(c);x.useEffect(()=>{g(c)},[c]);const b=f!==c,{items:v}=E("departments",W),{items:A}=E("providers",G),{items:u}=E("appointments",z),{items:t}=E("slot_overrides",V),n=x.useMemo(()=>re({clinics:v,providers:A,appointments:u,overrides:t,lang:l}),[v,A,u,t,l]),a=`${c.trim()}

${n}`.trim(),[p,w]=x.useState(!1),[S,$]=x.useState("idle"),M=async()=>{try{await navigator.clipboard.writeText(a),w(!0),setTimeout(()=>w(!1),1500)}catch{}},P=async()=>{$("sending");try{const I=await fetch("/api/demo/clinic/agent/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(h==="persona"?{persona:c}:h==="kb"?{kb:c}:{})});if(!I.ok)throw new Error(`HTTP ${I.status}`);$("ok"),setTimeout(()=>$("idle"),1500)}catch(_){console.error("Apply to agent failed:",_),$("err"),setTimeout(()=>$("idle"),2500)}};return s.jsxs("div",{className:"space-y-6",children:[s.jsxs("div",{className:"flex flex-wrap items-end justify-between gap-3",children:[s.jsxs("div",{children:[s.jsx("h1",{className:"text-2xl font-semibold tracking-tight text-foreground",children:o}),s.jsx("p",{className:"mt-1 max-w-3xl text-sm text-muted-foreground",children:r})]}),s.jsxs("div",{className:"flex items-center gap-2",children:[b?s.jsxs("span",{className:"text-xs text-amber-600 dark:text-amber-400",children:["● ",e("unsavedChanges")]}):s.jsx("span",{className:"text-xs text-muted-foreground",children:e("saved")}),s.jsxs(T,{variant:"outline",onClick:m,children:[s.jsx(K,{className:"me-2 h-4 w-4"}),e("resetToDefault")]}),s.jsxs(T,{onClick:()=>y(f),disabled:!b,children:[s.jsx(Q,{className:"me-2 h-4 w-4"}),e("save")]}),s.jsxs(T,{variant:"outline",onClick:P,disabled:S==="sending"||b,title:e(b?"unsavedChanges":"applyToAgent"),children:[s.jsx(oe,{className:"me-2 h-4 w-4"}),e(S==="ok"?"applied":S==="err"?"applyFailed":"applyToAgent")]})]})]}),s.jsx(D,{title:e("editableSection"),meta:`${f.length.toLocaleString()} chars`,children:s.jsx("div",{className:"p-4",children:s.jsx(F,{value:f,onChange:_=>g(_.target.value),rows:18,dir:"auto",className:"resize-y font-mono text-[12.5px] leading-relaxed"})})}),i&&s.jsx(D,{title:e("liveStatePreview"),meta:`${n.length.toLocaleString()} chars · auto`,children:s.jsx("pre",{className:"max-h-[420px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-muted-foreground",dir:"auto",children:n})}),s.jsx(D,{title:e("compiledPrompt"),meta:`${a.length.toLocaleString()} chars`,headerExtra:s.jsxs(T,{size:"sm",variant:"outline",onClick:_=>{_.stopPropagation(),M()},children:[p?s.jsx(Z,{className:"me-1.5 h-3.5 w-3.5"}):s.jsx(ee,{className:"me-1.5 h-3.5 w-3.5"}),e(p?"copied":"copyPrompt")]}),children:s.jsx("pre",{className:"max-h-[480px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-foreground",dir:"auto",children:a})})]})}function D({title:o,meta:r,headerExtra:h,children:d}){const[i,e]=x.useState(!1);return s.jsxs("div",{className:"overflow-hidden rounded-xl border border-border bg-card",children:[s.jsxs("button",{type:"button",onClick:()=>e(l=>!l),"aria-expanded":i,className:"flex w-full items-center justify-between gap-3 border-b border-border bg-card px-5 py-3 text-start hover:bg-accent/40 transition-colors",children:[s.jsxs("div",{className:"flex items-center gap-2",children:[s.jsx(te,{className:`h-4 w-4 text-muted-foreground transition-transform ${i?"rotate-0":"-rotate-90"}`}),s.jsx("h2",{className:"text-sm font-semibold text-card-foreground",children:o})]}),s.jsxs("div",{className:"flex items-center gap-3",children:[r&&s.jsx("span",{className:"text-xs text-muted-foreground",children:r}),h]})]}),i&&d]})}export{ve as D,Ae as P,Se as a};
