import{c as k,u as Y,j as i}from"./index-05RJjLfD.js";import{B as T}from"./button-0eRvbRDU.js";import{T as F}from"./textarea-DL2YwfRX.js";import{w as U,D as $,p as M,l as v,s as B,i as q,f as H,t as X,u as N,a as V,c as W,g as K,d as G}from"./demoStore-HPl1rdX7.js";import{R as Q}from"./rotate-ccw-RCbVWKTo.js";import{S as z}from"./save-D5eVjk4C.js";import{S as J}from"./send-DdKp6FkZ.js";import{C as Z}from"./check-Bm2ArBKG.js";import{C as ee}from"./copy-DdOzJ1oD.js";import{C as te}from"./chevron-down-DN9v-09-.js";const ae="pwdemo:clinic:doc",oe=1,I="pwdemo:clinic:doc-change";function O(o){return`${ae}:${o}:v${oe}`}function ne(o,r){const[h,d]=k.useState(()=>{if(typeof window>"u")return r;try{return localStorage.getItem(O(o))??r}catch{return r}});k.useEffect(()=>{if(typeof window>"u")return;const l=c=>{if(c.detail?.key===o)try{const u=localStorage.getItem(O(o));u!==null&&d(u)}catch{}};return window.addEventListener(I,l),()=>window.removeEventListener(I,l)},[o]);const s=k.useCallback(l=>{d(l);try{localStorage.setItem(O(o),l)}catch{}window.dispatchEvent(new CustomEvent(I,{detail:{key:o}}))},[o]),e=k.useCallback(()=>{d(r);try{localStorage.removeItem(O(o))}catch{}window.dispatchEvent(new CustomEvent(I,{detail:{key:o}}))},[o,r]);return{value:h,set:s,reset:e}}const ve=`# Primewave Mate Clinics — Riyadh
...
`;function ie({clinics:o,providers:r,appointments:h,overrides:d,lang:s}){const e=[],l=new Date,c=C(l),y=U(l);e.push("# Live clinic state (auto-generated — do NOT edit, refreshes per call)"),e.push(`Generated at: ${l.toISOString()}`),e.push(""),e.push(`## Clinics (${o.length} total)`);for(const t of o){const n=t.head_id?r.find(w=>w.id===t.head_id):null,a=(t.working_hours??$)[M(c)],m=a.open?`${a.open_time}–${a.close_time}${a.break_enabled?` (break ${a.break_start}–${a.break_end})`:""}`:"CLOSED today";e.push(`- ${v(t.name,t.name_ar,s)} · ${v(t.location,t.location_ar,s)} · ${v(t.specialty,t.specialty_ar,s)}`+(n?` · head: ${v(n.name,n.name_ar,s)}`:"")+` · today: ${m}`+(t.active?"":" · INACTIVE"))}e.push("");const u=r.filter(t=>t.active),f={};for(const t of u)f[t.role]=f[t.role]?[...f[t.role],t]:[t];e.push(`## Active staff (${u.length} total)`);for(const t of["doctor","nurse","tech","admin"]){const n=f[t]??[];if(n.length!==0){e.push(`- **${t}** (${n.length}):`);for(const a of n)e.push(`  - ${v(a.name,a.name_ar,s)} · ${v(a.specialty,a.specialty_ar,s)} · ${a.phone}`)}}e.push("");const g=ce(c,o,h,d);e.push(`## Today's totals (${c})`),e.push(`Across all clinics — slots ${g.totalSlots} · booked ${g.booked} · blocked ${g.blocked} · free ${g.free}`),e.push("");const b=7,_=6,S=[];{const t=new Date;t.setHours(0,0,0,0);for(let n=0;n<b;n++){const a=new Date(t);a.setDate(t.getDate()+n),S.push(C(a))}}e.push(`## Free slots — today and next ${b-1} days (per clinic)`);for(const t of o){e.push(`### ${v(t.name,t.name_ar,s)} — ${v(t.specialty,t.specialty_ar,s)}`);for(const n of S){const a=D(t,n,h,d),m=le(n,s);if(a.totalSlots===0){e.push(`- ${m}: closed`);continue}if(a.free===0){e.push(`- ${m}: FULL (${a.booked} booked${a.blocked>0?`, ${a.blocked} blocked`:""})`);continue}const w=a.freeSlots.slice(0,_).join(", "),x=a.freeSlots.length>_?` +${a.freeSlots.length-_} more`:"";e.push(`- ${m}: ${w}${x}  (${a.free} of ${a.totalSlots} free)`)}e.push("")}e.push("## This week totals (per clinic)");for(const t of o){let n=0,a=0,m=0;for(const w of y){const x=D(t,w,h,d);m+=x.totalSlots,n+=x.booked,a+=x.blocked}e.push(`- ${v(t.name,t.name_ar,s)}: slots ${m} · booked ${n} · blocked ${a} · free ${Math.max(0,m-n-a)}`)}e.push("");const p=d.filter(t=>t.date>=c);if(p.length>0){e.push(`## Active slot blocks (${p.length} dates)`);const t=[...p].sort((n,a)=>n.date.localeCompare(a.date));for(const n of t.slice(0,20)){const a=o.find(w=>w.id===n.department_id),m=a?v(a.name,a.name_ar,s):n.department_id;e.push(`- ${n.date} — ${m}: ${n.blocked_slots.length} slots blocked (${n.blocked_slots.slice(0,8).join(", ")}${n.blocked_slots.length>8?"…":""})`)}t.length>20&&e.push(`- …and ${t.length-20} more.`),e.push("")}return e.join(`
`)}function C(o){const r=o.getFullYear(),h=String(o.getMonth()+1).padStart(2,"0"),d=String(o.getDate()).padStart(2,"0");return`${r}-${h}-${d}`}const se=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],re=["الأحد","الإثنين","الثلاثاء","الأربعاء","الخميس","الجمعة","السبت"];function le(o,r){const[h,d,s]=o.split("-").map(g=>parseInt(g,10)),e=new Date(h,d-1,s),l=new Date;l.setHours(0,0,0,0);const c=Math.round((e.getTime()-l.getTime())/864e5),y=(r==="ar"?re:se)[e.getDay()],u=e.toLocaleDateString(r==="ar"?"ar-EG":void 0,{day:"numeric",month:"short"}),f=`${y} ${u}`;return c===0?`Today (${f})`:c===1?`Tomorrow (${f})`:f}function D(o,r,h,d){const s=(o.working_hours??$)[M(r)];if(!s.open)return{totalSlots:0,booked:0,blocked:0,free:0,freeSlots:[]};const e=B(s).filter(p=>!q(p,s)),l=H(h,r,o.id),c=new Set(d.filter(p=>p.department_id===o.id&&p.date===r).flatMap(p=>p.blocked_slots)),y=new Date,u=C(y),f=y.getHours()*60+y.getMinutes(),g=r===u,b=[];let _=0,S=0;for(const p of e){if(l.has(p)){_++;continue}if(c.has(p)){S++;continue}g&&X(p)<f||b.push(p)}return{totalSlots:e.length,booked:_,blocked:S,free:b.length,freeSlots:b}}function ce(o,r,h,d){let s=0,e=0,l=0,c=0;for(const y of r){const u=D(y,o,h,d);s+=u.totalSlots,e+=u.booked,l+=u.blocked,c+=u.free}return{totalSlots:s,booked:e,blocked:l,free:c}}function _e({heading:o,description:r,storageKey:h,defaultText:d,showLivePreview:s=!0}){const{t:e,lang:l}=Y(),{value:c,set:y,reset:u}=ne(h,d),[f,g]=k.useState(c);k.useEffect(()=>{g(c)},[c]);const b=f!==c,{items:_}=N("departments",V),{items:S}=N("providers",W),{items:p}=N("appointments",K),{items:t}=N("slot_overrides",G),n=k.useMemo(()=>ie({clinics:_,providers:S,appointments:p,overrides:t,lang:l}),[_,S,p,t,l]),a=`${c.trim()}

${n}`.trim(),[m,w]=k.useState(!1),[x,E]=k.useState("idle"),j=async()=>{try{await navigator.clipboard.writeText(a),w(!0),setTimeout(()=>w(!1),1500)}catch{}},P=async()=>{E("sending");try{const L=await fetch("/api/demo/clinic/agent/prompt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(h==="persona"?{persona:c}:h==="kb"?{kb:c}:{})});if(!L.ok)throw new Error(`HTTP ${L.status}`);E("ok"),setTimeout(()=>E("idle"),1500)}catch(A){console.error("Apply to agent failed:",A),E("err"),setTimeout(()=>E("idle"),2500)}};return i.jsxs("div",{className:"space-y-6",children:[i.jsxs("div",{className:"flex flex-wrap items-end justify-between gap-3",children:[i.jsxs("div",{children:[i.jsx("h1",{className:"text-2xl font-semibold tracking-tight text-foreground",children:o}),i.jsx("p",{className:"mt-1 max-w-3xl text-sm text-muted-foreground",children:r})]}),i.jsxs("div",{className:"flex items-center gap-2",children:[b?i.jsxs("span",{className:"text-xs text-amber-600 dark:text-amber-400",children:["● ",e("unsavedChanges")]}):i.jsx("span",{className:"text-xs text-muted-foreground",children:e("saved")}),i.jsxs(T,{variant:"outline",onClick:u,children:[i.jsx(Q,{className:"me-2 h-4 w-4"}),e("resetToDefault")]}),i.jsxs(T,{onClick:()=>y(f),disabled:!b,children:[i.jsx(z,{className:"me-2 h-4 w-4"}),e("save")]}),i.jsxs(T,{variant:"outline",onClick:P,disabled:x==="sending"||b,title:e(b?"unsavedChanges":"applyToAgent"),children:[i.jsx(J,{className:"me-2 h-4 w-4"}),e(x==="ok"?"applied":x==="err"?"applyFailed":"applyToAgent")]})]})]}),i.jsx(R,{title:e("editableSection"),meta:`${f.length.toLocaleString()} chars`,children:i.jsx("div",{className:"p-4",children:i.jsx(F,{value:f,onChange:A=>g(A.target.value),rows:18,dir:"auto",className:"resize-y font-mono text-[12.5px] leading-relaxed"})})}),s&&i.jsx(R,{title:e("liveStatePreview"),meta:`${n.length.toLocaleString()} chars · auto`,children:i.jsx("pre",{className:"max-h-[420px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-muted-foreground",dir:"auto",children:n})}),i.jsx(R,{title:e("compiledPrompt"),meta:`${a.length.toLocaleString()} chars`,headerExtra:i.jsxs(T,{size:"sm",variant:"outline",onClick:A=>{A.stopPropagation(),j()},children:[m?i.jsx(Z,{className:"me-1.5 h-3.5 w-3.5"}):i.jsx(ee,{className:"me-1.5 h-3.5 w-3.5"}),e(m?"copied":"copyPrompt")]}),children:i.jsx("pre",{className:"max-h-[480px] overflow-auto whitespace-pre-wrap p-4 font-mono text-[12px] leading-relaxed text-foreground",dir:"auto",children:a})})]})}function R({title:o,meta:r,headerExtra:h,children:d}){const[s,e]=k.useState(!1);return i.jsxs("div",{className:"overflow-hidden rounded-xl border border-border bg-card",children:[i.jsxs("button",{type:"button",onClick:()=>e(l=>!l),"aria-expanded":s,className:"flex w-full items-center justify-between gap-3 border-b border-border bg-card px-5 py-3 text-start hover:bg-accent/40 transition-colors",children:[i.jsxs("div",{className:"flex items-center gap-2",children:[i.jsx(te,{className:`h-4 w-4 text-muted-foreground transition-transform ${s?"rotate-0":"-rotate-90"}`}),i.jsx("h2",{className:"text-sm font-semibold text-card-foreground",children:o})]}),i.jsxs("div",{className:"flex items-center gap-3",children:[r&&i.jsx("span",{className:"text-xs text-muted-foreground",children:r}),h]})]}),s&&d]})}export{ve as D,_e as P,ke as a};
